...
if (!isset($_SESSION['auth'])) {
    // If they are trying to access anything other than the 'login' page (or root, which App defaults to 'home')
    // We explicitly redirect to the login page.
    if ($current_controller !== 'login') {
        header('Location: /login'); // Redirect to clean login URL
        exit(); // Crucial: Terminate script execution after redirect
    }
    // If current_controller IS 'login', then we allow the login view to load below.
} 
// CONDITION: If user IS authenticated
    ...
    if ($current_controller === 'login' || $current_controller === '') {
        header('Location: /home'); // Redirect to clean home URL
        exit(); // Crucial: Terminate script execution after redirect
    }
}
?>
